...
export default {
    ...
    setup(props,{emit}){
      ...
      let updatedcustomer = reactive({ 
        ...
        updatedcustomer=computed(()=>{
            return {...(store.getters.gettobeuptadedCustomer)}
        });
        const guncelle=()=>{
            store.dispatch('cleanAndUpdateCustomer',updatedcustomer.value);
            store.dispatch('changeDisplayModal',false)
            updatedcustomer={}
        };
        const kapat=()=> {
        ...
        const format_date=(value)=> {
          ...
          return{
            updatedcustomer,guncelle,kapat,format_date
          }
    },
